...
function addExpDate(id){
	...
	const xhr = new XMLHttpRequest();
	xhr.open('PUT','http://localhost:8080/products/exp-date/'+id+'?date='+date+'&days='+reminderDays+'&note='+note);
		xhr.responseType = 'json';
		xhr.setRequestHeader('Content-Type', 'application/json');
		xhr.onload = () =>{
			expDates(id);
...
function removeExpDate(string){
	...
	var date=string[1];
	const xhr = new XMLHttpRequest();
	xhr.open('DELETE','http://localhost:8080/products/exp-date/'+id+'?expirationDateId='+date);
		xhr.responseType = 'json';
		xhr.setRequestHeader('Content-Type', 'application/json');
		xhr.onload = () =>{
			expDates(id);
